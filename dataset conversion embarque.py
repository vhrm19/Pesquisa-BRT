...
for i in range(len(Tempo_por_Passageiro)):
    Tempo_por_Passageiro[i] = (Tempo_por_Passageiro[i] - 2.446792929) / 2.679165479
    Passageiros[i] = (Passageiros[i] - 2.866666667) / 2.111041727
    Em_pe[i] = (Em_pe[i] - 1.783333333) / 0.865372785
for i in range(len(dataset)):
    dataset[i][0] = str((float(Tempo_por_Passageiro[i])))
    dataset[i][1] = str((float(Passageiros[i])))
    dataset[i][2] = str((float(Em_pe[i])))

arq = open('csv embarque padronizado.csv', 'w')
