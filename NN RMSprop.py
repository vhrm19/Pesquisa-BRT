...
S = np.zeros((len(W[0]),len(W[0].T)))

for j in range(100000): # Faz o Backpropagation minimizando a função custo: 0.5 * sum(y-ŷ)**2, de acordo com o shape da NN
    a, Z = Forward_Propagation(Entrada, layers, W)
    Grad = Backpropagation(Tempo_por_Passageiro, a, Z, W)
    for i in range(len(W)):
        S = (0.9 * sum(S) + 0.1 * sum(Grad[i])**2)
        W[i] -= (0.001 / np.sqrt(S + 10**-6)) * Grad[i]
